...
function GetLoadNav() {
    ...
    var activeMenuId = "";
    var activeParentMenuId = "";
    var activeChildCount = 1;
        ...
        if (row.ParentId == "0") {
            ...
            if (childNodes.length > 0) {
                ...
                $.each(childNodes, function (i) {
                    ...
                    if (subrow.UrlAddress.startWith(currUrl)) {
                        activeMenuId = subrow.Id;
                        activeParentMenuId = row.Id;
                        activeChildCount = childNodes.length;
                        navCookies(subrow.Id);
                    }
                });
    ...
    if (activeParentMenuId.length > 0) {
        var $activeParentMenu = $("a[data-id='" + activeParentMenuId + "']");
        if ($activeParentMenu) {
            $activeParentMenu.parent().addClass("open");
            var $activeMenu = $("a[data-id='" + activeMenuId + "']");
            //$activeMenu.parent().parent('.submenu').slideUp('fast');
            $activeMenu.parent().parent('.submenu').css({
                "display": "block", "overflow": "auto", "height": 40 * activeChildCount+"px"
            });
            //display: block; overflow: auto; height: 124px;
            $activeMenu.parent().find('li').addClass('open');
        }
    }
...
function navCookies(dataId)
{
    //var dataId = $("#sidebar-nav ul li").attr('data-id')    
    if (dataId != "") {
        ...
        if ($(".btntool") != undefined && $(".btntool").length > 0) {
            $('.btntool').authorizeButton();
        }
    }
}
